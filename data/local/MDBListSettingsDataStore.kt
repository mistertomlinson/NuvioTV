...
class MDBListSettingsDataStore @Inject constructor(
    ...
    private val showOnDetailScreenKey = booleanPreferencesKey("mdblist_show_on_detail_screen")
    private val showOnHomeScreenKey = booleanPreferencesKey("mdblist_show_on_home_screen")
    ...
    val settings: Flow<MDBListSettings> = profileManager.activeProfileId.flatMapLatest { pid ->
        factory.get(pid, FEATURE).data.map { prefs ->
            MDBListSettings(
                ...
                showOnDetailScreen = prefs[showOnDetailScreenKey] ?: true,
                showOnHomeScreen = prefs[showOnHomeScreenKey] ?: false
            )
        }
    }
    ...
    suspend fun setShowOnDetailScreen(enabled: Boolean) {
        store().edit { it[showOnDetailScreenKey] = enabled }
    }

    suspend fun setShowOnHomeScreen(enabled: Boolean) {
        store().edit { it[showOnHomeScreenKey] = enabled }
    }
}
