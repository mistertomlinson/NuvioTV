...
class ContinueWatchingEnrichmentCache @Inject constructor(
    ...
    private val nextUpMutex = Mutex()
    private val inProgressMutex = Mutex()
     ...
     */
    suspend fun saveNextUpSnapshot(items: List<CachedNextUpItem>, force: Boolean = false, profileId: Int = profileManager.activeProfileId.value) = withContext(Dispatchers.IO) {
     ...
     * @param force bypass throttle and content-change check (use at end of pipeline or for clears)
     */
    suspend fun saveInProgressSnapshot(items: List<CachedInProgressItem>, force: Boolean = false, profileId: Int = profileManager.activeProfileId.value) = withContext(Dispatchers.IO) {
        val contentHash = items.hashCode()
        if (!force) {
    ...
    suspend fun clearAll() = withContext(Dispatchers.IO) {
        nextUpMutex.withLock { inProgressMutex.withLock {
            try {
                nextUpFile().delete()
