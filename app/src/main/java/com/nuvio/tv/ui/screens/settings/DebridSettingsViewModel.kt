...
class DebridSettingsViewModel @Inject constructor(
    ...
    fun setInstantPlaybackPreparationEnabled(enabled: Boolean) {
        val nextLimit = if (enabled) DEBRID_PREPARE_INSTANT_PLAYBACK_DEFAULT_LIMIT else 0
    ...
    suspend fun startDeviceAuthorization(providerId: String): DebridDeviceAuthorization? {
        return when (DebridProviders.byId(providerId)?.id) {
            DebridProviders.TORBOX_ID -> {
                ...
                val data = response.body()?.takeIf { response.isSuccessful && it.success != false }?.data ?: return null
    ...
    suspend fun redeemDeviceAuthorization(providerId: String, deviceCode: String): DebridDeviceAuthorizationTokenResult {
        ...
        return when (DebridProviders.byId(providerId)?.id) {
            DebridProviders.TORBOX_ID -> {
                ...
                torboxTokenResult(response)
            }
            DebridProviders.PREMIUMIZE_ID -> {
                val clientId = premiumizeClientIdOrThrow()
                val response = premiumizeApi.redeemDeviceAuthorization(deviceCode = normalized, clientId = clientId)
                premiumizeTokenResult(response)
            }
    ...
    private fun torboxTokenResult(
        response: retrofit2.Response<TorboxEnvelopeDto<TorboxDeviceTokenDto>>
    ): DebridDeviceAuthorizationTokenResult {
        val envelope = response.body()
        val accessToken = envelope?.takeIf { response.isSuccessful && it.success != false }
            ?.data?.accessToken?.takeIf { it.isNotBlank() }
        if (accessToken != null) return DebridDeviceAuthorizationTokenResult.Authorized(accessToken)
        ...
        return when {
            message.contains("pending") || message.contains("not authorized") ||
                ...
                message.contains("scan the code") -> DebridDeviceAuthorizationTokenResult.Pending
            message.contains("expired") -> DebridDeviceAuthorizationTokenResult.Expired
            response.code() in listOf(404, 409, 425) -> DebridDeviceAuthorizationTokenResult.Pending
            response.code() == 410 -> DebridDeviceAuthorizationTokenResult.Expired
            else -> DebridDeviceAuthorizationTokenResult.Failed(envelope?.detail ?: envelope?.error)
        }
    }

    private fun premiumizeTokenResult(
        response: retrofit2.Response<PremiumizeDeviceTokenDto>
    ): DebridDeviceAuthorizationTokenResult {
        val body = response.body()
        body?.accessToken?.takeIf { response.isSuccessful && it.isNotBlank() }?.let {
            return DebridDeviceAuthorizationTokenResult.Authorized(it)
        }
        return when (body?.error?.lowercase()) {
            ...
            else -> if (response.code() == 400 && body?.error.isNullOrBlank()) {
...
data class DebridSettingsUiState(
    ...
    val resolverProviders: List<com.nuvio.tv.core.debrid.DebridProvider>
        get() = DebridProviders.visible().filter { provider ->
    ...
    val hasResolverProvider: Boolean get() = activeResolverProvider != null

    val hasCloudLibraryProvider: Boolean
        get() = DebridProviders.visible().any { provider ->
            apiKeyFor(provider.id).isNotBlank() && provider.supports(DebridProviderCapability.CloudLibrary)
        }

    val canResolvePlayableLinks: Boolean get() = enabled && hasResolverProvider
    val canUseCloudLibrary: Boolean get() = cloudLibraryEnabled && hasCloudLibraryProvider

    fun apiKeyFor(providerId: String): String = providerApiKeys[providerId].orEmpty()
