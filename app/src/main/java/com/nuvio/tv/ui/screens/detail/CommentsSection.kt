        ...
        Box(
            modifier = Modifier
                ...
                .onPreviewKeyEvent { event ->
                    if (event.type != KeyEventType.KeyDown) {
                        ...
                        when (event.key) {
                            Key.DirectionLeft -> {
                                if (canNavigatePrevious) { onPrevious(); true } else false
                            }
                            Key.DirectionRight -> {
                                if (canNavigateNext) { if (!isLoadingNext) onNext(); true } else false
                            }
                            else -> false
                        }
                    }
            ...
            Column(
                modifier = Modifier
                    ...
                    .focusable()
                    .focusProperties { down = mainContentFocusRequester },
    ...
    Column(
        ...
        Column(
            ...
            Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                ...
                review.authorUsername?.takeIf { it.isNotBlank() }?.let { username ->
                    Text(
                        text = "@$username",
            ...
            Box(
                modifier = Modifier
                    ...
                    .focusable()
                    .focusProperties { up = primaryFocusRequester }
                    .onPreviewKeyEvent { event ->
                        when {
