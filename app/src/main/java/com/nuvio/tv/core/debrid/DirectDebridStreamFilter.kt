...
object DirectDebridStreamFilter {
    ...
    fun filterInstant(streams: List<Stream>, settings: DebridSettings? = null): List<Stream> {
        val instantStreams = streams
            ...
            .map { stream ->
                ...
                stream.copy(name = stream.name ?: sourceName, addonName = sourceName, addonLogo = null)
            }
            .distinctBy { stream ->
                listOf(
    ...
    fun applyPreferences(streams: List<Stream>, settings: DebridSettings): List<Stream> {
        ...
        val orderedStreams = if (preferences.sortCriteria.isEmpty()) matchedStreams
        else matchedStreams.sortedWith { left, right -> compareFacts(left.second, right.second, preferences.sortCriteria) }
        return applyLimits(orderedStreams, preferences).map { it.first }
    }
    ...
    private fun effectivePreferences(settings: DebridSettings): DebridStreamPreferences {
        ...
        if (settings.streamMaxResults == 0 && settings.streamSortMode == DebridStreamSortMode.DEFAULT &&
            ...
            settings.streamCodecFilter == DebridStreamCodecFilter.ANY) return default
    ...
    private fun applyLimits(streams: List<Pair<Stream, StreamFacts>>, preferences: DebridStreamPreferences): List<Pair<Stream, StreamFacts>> {
        ...
        for (stream in streams) {
            ...
            if (preferences.maxPerResolution > 0 && (resolutionCounts[stream.second.resolution] ?: 0) >= preferences.maxPerResolution) continue
            if (preferences.maxPerQuality > 0 && (qualityCounts[stream.second.quality] ?: 0) >= preferences.maxPerQuality) continue
    ...
    private fun compareFacts(left: StreamFacts, right: StreamFacts, criteria: List<DebridStreamSortCriterion>): Int {
        for (criterion in criteria) { val c = compareKey(left, right, criterion); if (c != 0) return c }
        return 0
    }

    private fun compareKey(left: StreamFacts, right: StreamFacts, criterion: DebridStreamSortCriterion): Int {
        val d = if (criterion.direction == DebridStreamSortDirection.ASC) 1 else -1
        return when (criterion.key) {
            DebridStreamSortKey.RESOLUTION -> left.resolutionRank.compareTo(right.resolutionRank) * -d
            DebridStreamSortKey.QUALITY -> left.qualityRank.compareTo(right.qualityRank) * -d
            DebridStreamSortKey.VISUAL_TAG -> left.visualRank.compareTo(right.visualRank) * -d
            DebridStreamSortKey.AUDIO_TAG -> left.audioRank.compareTo(right.audioRank) * -d
            DebridStreamSortKey.AUDIO_CHANNEL -> left.channelRank.compareTo(right.channelRank) * -d
            DebridStreamSortKey.ENCODE -> left.encodeRank.compareTo(right.encodeRank) * -d
            DebridStreamSortKey.SIZE -> (left.size ?: 0L).compareTo(right.size ?: 0L) * d
            DebridStreamSortKey.LANGUAGE -> left.languageRank.compareTo(right.languageRank) * -d
    ...
    private fun streamFacts(stream: Stream, preferences: DebridStreamPreferences): StreamFacts {
        ...
        return StreamFacts(
            resolution = resolution, quality = quality, visualTags = visualTags,
            audioTags = audioTags, audioChannels = audioChannels, encode = encode,
            languages = languages, releaseGroup = releaseGroup, size = streamSize(stream),
    ...
    private fun resolutionValue(value: String?): DebridStreamResolution? {
        val n = value?.lowercase().orEmpty()
        return when {
            n.hasResolutionToken("2160p?", "4k", "uhd") -> DebridStreamResolution.P2160
            n.hasResolutionToken("1440p?", "2k") -> DebridStreamResolution.P1440
            n.hasResolutionToken("1080p?", "fhd") -> DebridStreamResolution.P1080
            n.hasResolutionToken("720p?", "hd") -> DebridStreamResolution.P720
            n.hasResolutionToken("576p?") -> DebridStreamResolution.P576
            n.hasResolutionToken("480p?", "sd") -> DebridStreamResolution.P480
            n.hasResolutionToken("360p?") -> DebridStreamResolution.P360
            else -> null
        }
    }

    private fun streamQuality(parsedQuality: String?, searchText: String): DebridStreamQuality {
        val t = listOfNotNull(parsedQuality, searchText).joinToString(" ").lowercase()
        return when {
            t.contains("remux") -> DebridStreamQuality.BLURAY_REMUX
            t.contains("blu-ray") || t.contains("bluray") || t.contains("bdrip") || t.contains("brrip") -> DebridStreamQuality.BLURAY
            t.contains("web-dl") || t.contains("webdl") -> DebridStreamQuality.WEB_DL
            t.contains("webrip") || t.contains("web-rip") -> DebridStreamQuality.WEBRIP
            t.contains("hdrip") -> DebridStreamQuality.HDRIP
            t.contains("hd-rip") || t.contains("hcrip") -> DebridStreamQuality.HD_RIP
            t.contains("dvdrip") -> DebridStreamQuality.DVDRIP
            t.contains("hdtv") -> DebridStreamQuality.HDTV
            t.hasToken("cam") -> DebridStreamQuality.CAM
            t.hasToken("ts") -> DebridStreamQuality.TS
            t.hasToken("tc") -> DebridStreamQuality.TC
            t.hasToken("scr") -> DebridStreamQuality.SCR
    ...
    private fun String.isDolbyVisionToken(): Boolean {
        val n = lowercase().replace(Regex("[^a-z0-9]"), "")
        return n == "dv" || n == "dovi" || n == "dolbyvision"
    }

    private fun String.isHdrToken(): Boolean {
        val n = lowercase().replace(Regex("[^a-z0-9+]"), "")
        return n == "hdr" || n == "hdr10" || n == "hdr10+" || n == "hdr10plus" || n == "hlg"
    }

    private fun streamSize(stream: Stream): Long? =
        stream.clientResolve?.stream?.raw?.size ?: stream.behaviorHints?.videoSize ?: stream.debridCacheStatus?.cachedSize
    ...
    private fun streamSearchText(stream: Stream): String {
        ...
        return listOfNotNull(
            ...
            resolve?.torrentName, resolve?.filename, raw?.torrentName, raw?.filename,
            stream.debridCacheStatus?.cachedName, parsed?.resolution, parsed?.quality,
            parsed?.codec, parsed?.hdr?.joinToString(" "), parsed?.audio?.joinToString(" ")
        ).joinToString(" ").lowercase()
    }

    private fun sourceName(stream: Stream): String = DebridProviders.instantName(stream.clientResolve?.service)
