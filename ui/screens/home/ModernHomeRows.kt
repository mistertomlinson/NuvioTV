...
internal fun ModernRowSection(
    ...
    Column {
        ...
        val numberedRowSpacing = if (isNumbered) {
            when {
                row.items.size >= 100 -> (modernCatalogCardWidth * 0.95f).coerceAtLeast(12.dp)
                row.items.size >= 10 -> (modernCatalogCardWidth * 0.80f).coerceAtLeast(12.dp)
                else -> (modernCatalogCardWidth * 0.38f).coerceAtLeast(12.dp)
            }
        } else 12.dp
        val numberedRowStartPadding = if (isNumbered) (modernCatalogCardWidth * 1.0f).coerceAtLeast(52.dp) else rowStartPadding

        // Pre-measure number widths once at row level for stable sizing across all items
        val numberFontSize = androidx.compose.ui.unit.TextUnit(modernCatalogCardHeight.value * 0.55f, androidx.compose.ui.unit.TextUnitType.Sp)
        val numberBaseStyle = androidx.compose.ui.text.TextStyle(
            fontSize = numberFontSize,
            fontWeight = androidx.compose.ui.text.font.FontWeight.Black,
            fontFamily = com.nuvio.tv.ui.theme.InterFamily
        )
        val rowTextMeasurer = androidx.compose.ui.text.rememberTextMeasurer()
        val singleDigitWidth = remember(numberBaseStyle) {
            rowTextMeasurer.measure("8", numberBaseStyle).size.width
        }
        val doubleDigitWidth = remember(numberBaseStyle) {
            rowTextMeasurer.measure("88", numberBaseStyle).size.width
        }
        val tripleDigitWidth = remember(numberBaseStyle) {
            rowTextMeasurer.measure("888", numberBaseStyle).size.width
        }
                        ...
                        is ModernPayload.Catalog -> {
                            ...
                            if (isNumbered) {
                                val digitPadding = when {
                                    cardNumber >= 100 -> modernCatalogCardWidth * 0.50f
                                    cardNumber >= 10 -> modernCatalogCardWidth * 0.25f
                                    else -> 0.dp
                                }
...
private fun NumberedCatalogCardWrapper(
    ...
    // Base style using app font
    val baseStyle = androidx.compose.ui.text.TextStyle(
        fontSize = numberFontSize,
        fontWeight = androidx.compose.ui.text.font.FontWeight.Black,
        fontFamily = com.nuvio.tv.ui.theme.InterFamily
    )
    // Outline effect: draw slightly larger dark text behind, then white text on top
    // This avoids stroke artifacts on glyphs with internal path intersections (e.g. 4, 8)
    ...
    val fillStyle = baseStyle.copy(color = androidx.compose.ui.graphics.Color(0xFF000000))

    // Use pre-measured width from row level for stable, consistent sizing
    val referenceText = when {
        number >= 100 -> "888"
        number >= 10 -> "88"
        else -> "8"
    }
    val measured = remember(referenceText, baseStyle) { textMeasurer.measure(referenceText, baseStyle) }
    ...
    // Right edge of number tucks under card by this amount
    val overlapDp = cardWidth * 0.10f
    // Number sits to the left of card; left edge = -(textWidthDp - overlapDp)
    ...
    Box(modifier = androidx.compose.ui.Modifier.padding(start = extraStartPadding)) {
        Box(
            modifier = androidx.compose.ui.Modifier
                ...
                .offset(x = offsetX, y = 18.dp)
                .zIndex(-1f)
                .layout { measurable, constraints ->
                    // Measure at full text width but report zero width to parent
                    // so the number never affects LazyRow item spacing
                ...
                .drawWithCache {
                    val outlineStyle = baseStyle.copy(color = androidx.compose.ui.graphics.Color(0xFFAAAAAA))
                    val outlineDarkStyle = baseStyle.copy(color = androidx.compose.ui.graphics.Color(0xFF666666))
                    val fillMeasured = textMeasurer.measure(numberText, fillStyle)
                    val outlineMeasured = textMeasurer.measure(numberText, outlineStyle)
                    val outlineDarkMeasured = textMeasurer.measure(numberText, outlineDarkStyle)
                    onDrawWithContent {
                        val textW = outlineMeasured.size.width.toFloat()
                        android.util.Log.d("NumDraw", "size.width=${size.width} textW=$textW")
                        val startX = size.width - textW
                        val o = 2f * density.density
                        // Light grey on left/top edges
                        for ((dx, dy) in listOf(-o to -o, -o to 0f, -o to o, 0f to -o)) {
                            drawText(outlineMeasured, topLeft = androidx.compose.ui.geometry.Offset(startX + dx, dy))
                        }
                        // Dark grey on right/bottom edges (shadow side toward poster)
                        for ((dx, dy) in listOf(o to o, o to 0f, o to -o, 0f to o)) {
                            drawText(outlineDarkMeasured, topLeft = androidx.compose.ui.geometry.Offset(startX + dx, dy))
                        }
                        // Pure black fill on top covers all interior bleed
                        drawText(fillMeasured, topLeft = androidx.compose.ui.geometry.Offset(startX, 0f))
                    }
                }
        ) {}
