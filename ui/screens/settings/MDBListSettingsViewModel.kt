...
class MDBListSettingsViewModel @Inject constructor(
    ...
    fun onEvent(event: MDBListSettingsEvent) {
        when (event) {
            ...
            is MDBListSettingsEvent.ToggleShowOnDetailScreen -> update { dataStore.setShowOnDetailScreen(event.enabled) }
            is MDBListSettingsEvent.ToggleShowOnHomeScreen -> update { dataStore.setShowOnHomeScreen(event.enabled) }
        }
    }
...
data class MDBListSettingsUiState(
    ...
    val showOnDetailScreen: Boolean = true,
    ...
    fun fromSettings(settings: MDBListSettings): MDBListSettingsUiState = copy(
        ...
        showOnDetailScreen = settings.showOnDetailScreen,
        showOnHomeScreen = settings.showOnHomeScreen
    )
}
...
sealed class MDBListSettingsEvent {
    ...
    data class ToggleShowOnDetailScreen(val enabled: Boolean) : MDBListSettingsEvent()
    data class ToggleShowOnHomeScreen(val enabled: Boolean) : MDBListSettingsEvent()
}
